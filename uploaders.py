...
def send_twitter(proj_id, stat, user_coid, user_uid, access_token, access_token_secret):
    ...
    VIDEO_FILENAME =  os.path.join(
        Config.BASE_DIR,
        Config.VIDS_LOCATION, 
        str(proj_id), 
        str(proj_id)+'_edited.mp4'
    )
    ...
    logging.debug("JSON Status: {}".format(req.json()))
...
def send_youtube(proj_id, youtube_credentials, user_coid, user_uid, yt_title, desc, yt_tags, privacy):
    ...
    yt_status = resumable_upload(insert_request, proj_id)
