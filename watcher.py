...
class Watcher:
    DIRECTORY_TO_WATCH = os.path.join(Config.BASE_DIR, Config.UPLOAD_QUEUE)
    ...
    def on_any_event(event):
        ...
        # Linux is modified
            ...
            if json_data['status'] == False:
                ...
                logging.debug("Uploading to {}".format(upload_type))
                if upload_type == 'youtube':
                    upload_status = uploaders.send_youtube(
                        proj_id,
                        json_data['youtube_credentials'],
                        json_data['coid'],
                        json_data['uid'],
                        json_data['yt_title'],
                        json_data['yt_desc'],
                        json_data['yt_tags'],
                        json_data['yt_privacy']
                    )
                elif upload_type == 'twitter':
                    upload_status = uploaders.send_twitter(
                        proj_id,
                        json_data['twitter_body'],
                        json_data['coid'],
                        json_data['uid'],
                    ...
                    upload_status = uploaders.send_facebook(
                        proj_id,                        
                        json_data['access_token'],      
                        json_data['access_token_secret'],  
                        json_data['facebook_title'],
                        json_data['facebook_body'],
                        json_data['uid'],
                        json_data['coid']
                    )
                else:
...
if __name__ == '__main__':
    ...
    log_file_name = os.path.join(
        ...
        Config.UPLOAD_LOGS, 
        datetime.now().strftime("%Y.%m.%d-%H-%M-%S") + "_upload_watcher_instance.log"
    )
    ...
    logging.debug("This watcher service is for the authentication service, to upload videos to server")
    w = Watcher()
    w.run()
